...
public class TransController : MonoBehaviour
{
    ...
    public float Gain_angle_to_translation = 0.3f;
    ...
    protected float filteredTranslationValue = 0;
    ...
    void Update()
    {
        ...
        translationValue = tiltValue * Gain_angle_to_translation;

        //Low pass filter
        filterDataQueue.Enqueue(translationValue);        
        if(filterDataQueue.Count > filterLength )
        { filterDataQueue.Dequeue(); }
        foreach (float f in filterDataQueue)
            filteredTranslationValue += f;
        filteredTranslationValue /= filterLength;

        transform.position = new Vector3(filteredTranslationValue, 0, 0);
